...
def rsd_trackback_discovery(url: str):
    ...
    trackback_url = soup.find("trackback:ping")

    if not trackback_url:
        raise TrackbackError("No trackback URL found in RSD.")
...
def send_trackback(target_url, post_url, title: str = None, excerpt: str = None, blog_name: str = None):
    """Send a trackback to a URL.

    :param target_url: The URL to send the trackback to.
    :param post_url: The URL of your post.
    ...
    try:
        send_trackback_request = requests.post(
            target_url,
            data={"url": post_url, "title": title, "excerpt": excerpt, "blog_name": blog_name},
            headers={
